...
├── domain/         model/ (CanChi, DailyMetadata, …) · model/ads/ (AdUnit, AdUnitState) · repository (interface) · usecase
...
- `domain/model/ads/AdUnit.kt` — abstract class (`id`, `name`, `StateFlow<AdUnitState>`); contract `load/pause/resume/destroy`.
- `domain/model/ads/AdUnitState.kt` — enum `NONE · LOADING · LOADED · FAILED · IMPRESSION`; companion giữ delay constants.
...
`BaseViewModel` cung cấp: `state: StateFlow<S>`, `effect: SharedFlow<E>`, `updateState { }`, `suspend sendEffect()`.
...
- `XxxState.kt` / `XxxIntent.kt` / `XxxEffect.kt` — mỗi loại nằm trong file riêng cùng thư mục screen.

### Stability

State `@Immutable`; list `ImmutableList<T>`; UiModel là `@Immutable data class`.
