...
### Coding rules (toàn bộ `ui/`, `navigation/`, `widget/`)
...
Hilt · LiveData · XML layout · `GlobalScope` · `Thread.sleep` · `Log.*` (dùng Timber) · `koinViewModel` trong Screen (dùng `koinActivityViewModel`) · dark theme · landscape · hardcoded hex · `List<T>` trong State (dùng `ImmutableList`) · gọi Repository trực tiếp từ ViewModel (phải qua UseCase).
...
├── ui/          base/BaseViewModel · model · screens/{today,calendar,settings} · components
...
- `NavBackStack` quản lý ở `AppScaffold`.
