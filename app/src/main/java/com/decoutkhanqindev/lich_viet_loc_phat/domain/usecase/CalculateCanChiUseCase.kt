...
class CalculateCanChiUseCase(private val repository: CalendarRepository) {
    suspend operator fun invoke(date: SolarDate): Result<CanChi> =
        runCatching { repository.calculateCanChi(date) }
}
